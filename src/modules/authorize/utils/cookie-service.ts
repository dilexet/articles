...
export class CookieTokenService {
    ...
    setTokenToCookie(res: Response, tokens: ITokenType): Response {
        const jwtAccessCookieName: string = this.config.get<string>('JWT_ACCESS_COOKIE_NAME') as string;

        const jwtRefreshCookieName: string = this.config.get<string>('JWT_REFRESH_COOKIE_NAME') as string;
