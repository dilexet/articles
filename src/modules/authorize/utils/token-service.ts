...
export class TokenService {
  ...
  async generateTokensAsync(user: UserEntity): Promise<ITokenType | null> {
    ...
    try {
      ...
      const refreshTokenExpiresInMilliseconds = ms(
        config.refreshTokenExpiresIn,
      );
  ...
  async validateRefreshTokenAsync(
    token: string,
  ): Promise<ITokenPayloadType | null> {
    const config = {
      accessTokenSecret: this.config.get<string>('JWT_ACCESS_SECRET'),
    ...
    try {
      const refreshTokenData =
        await this.jwtService.verifyAsync<ITokenPayloadType>(token, {
          secret: config.refreshTokenSecret,
        });
