...
export class AuthorizeController {
    ...
    async refresh(@Req() req: Request, @Res() res: Response) {
        try {
            const refreshToken: string | null = this.cookieTokenService.getRefreshTokenFromCookie(req);

            if (!refreshToken) {
                return res.status(HttpStatus.UNAUTHORIZED).json({ message: 'Refresh token missing' });
            }

            const result = await this.authorizeService.refreshTokenAsync(refreshToken);
            ...
            return res.status(HttpStatus.OK).json({ message: 'Tokens refreshed successfully' });
        } catch (error) {
            console.log(error);
